...
import pro.network.mpchicken.cart.CartActivity;
import pro.network.mpchicken.chip.CategoryAdapter;
import pro.network.mpchicken.chip.ChipBean;
import pro.network.mpchicken.chip.OnChip;
import pro.network.mpchicken.orders.MyOrderPage;
import pro.network.mpchicken.product.CategoryBeen;
import pro.network.mpchicken.product.ProductActivity;
import pro.network.mpchicken.product.ProductItemClick;
import pro.network.mpchicken.product.ProductListAdapter;
import pro.network.mpchicken.product.ProductListBean;
import pro.network.mpchicken.wallet.WalletActivity;

import static pro.network.mpchicken.app.AppConfig.NEW_CATEGORY;
import static pro.network.mpchicken.app.AppConfig.SETTING;
import static pro.network.mpchicken.app.AppConfig.districtKey;
...
public class HomeActivity extends BaseActivity implements ProductItemClick, OnChip {
    ...
    protected void startDemo() {
        ...
        product_descrpition = findViewById(R.id.description);
        readMoreString = (String) getResources().getText(R.string.read_more);
        ...
        phone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    intent.setData(Uri.parse("http://api.whatsapp.com/send?phone=918883438888"
                            + "&text=" + "Hi MP Chicken"));
    ...
    private void getAllCategories() {
        ...
        StringRequest strReq = new StringRequest(Request.Method.GET,
                NEW_CATEGORY, new Response.Listener<String>() {
            @Override
    ...
    private void getAllSettings() {
        ...
        String tag_string_req = "req_register";
        StringRequest strReq = new StringRequest(Request.Method.GET,
                SETTING+"?district="+ sharedpreferences.getString(districtKey, ""),
    ...
    private void getFromServer(final String searchKey) {
        ...
        // showDialog();
        StringRequest strReq = new StringRequest(Request.Method.GET,
                AppConfig.STOCK+"?searchKey="+searchKey, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
    ...
    private void fetchBanner() {
        ...
        // showDialog();
        StringRequest strReq = new StringRequest(Request.Method.GET,
                AppConfig.BANNER, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Log.d("Register Response: ", response);
                try {
                        ...
                        if (jObj.has("enabled") &&
                            ...
                            if (jObj.has("title")) {
                                ...
                                if (jObj.has("descriptionAn")) {
                                    product_descrpition.setText(jObj.getString("descriptionAn"));
                                }
                            }
